...
export default function Programcilar() {
  ...
  const [programciİd , setProgramciİd] = useState(null);
 ...
 let isim = "";
  programcilar.forEach(item => {
   if(programciİd === item.id )isim = item.isim   
  ...
  const stil = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color: programciİd === null ? 'royalblue' : "gold" , // 🤔 kutlarken renk gold'a dönecek
  };
      ...
      <div className='programmers'>
        {
          ...
          programcilar.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.isim} <button onClick={() => {setProgramciİd(dev.id)}}>Kutla</button>
            </div>
          )
        }
      </div>
